...
- **投屏后系统界面能点、游戏内点击无效**：OPPO/小米等 ROM 需开启开发者选项里的「**USB调试（安全设置）**」（不同品牌叫法略有差异，小米/OPPO 均为该名称）。未开启时，模拟点击无法注入第三方应用（系统桌面/Home 不受影响），表现为投屏点击在游戏内无反应。注意：部分 OPPO 机型开启该开关需插入 SIM 卡并登录 OPPO 账号验证身份；验证通过后拔掉 SIM 卡开关依然保持开启。
